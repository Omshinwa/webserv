        ...
        private:
            ...
            LocationConfig  parse_location() {
                ...
                if (peek().type != Token::WORD)
                    fail("expected path after 'location'");
                loc.path = peek().value;
                ++_pos;
                ...
                while (peek().type != Token::CLOSE_BRACE) {
                    ...
                    if (name == "methods" || name == "allow_methods" || name == "limit_except") {
                        if (args.empty())
                            fail("'methods' expects at least one method");
                        loc.methods = args;
                        loc.has_methods = true;
                    }
                    else if (name == "root") {
                        if (args.size() != 1)
                            fail("'root' expects one argument");
                        loc.root = args[0];
                        loc.has_root = true;
                    }
                    else if (name == "index") {
                        if (args.size() != 1)
                            fail("'index' expects one argument");
                        loc.index = args[0];
                        loc.has_index = true;
                    }
                        ...
                        if (args.size() != 1)
                            fail("'upload_dir' expects one argument");
            ...
            ServerConfig    parse_server() {
                ...
                while (peek().type != Token::CLOSE_BRACE) {
                    ...
                    if (name == "listen") {
                        if (listen_seen)
                            fail("duplicate 'listen' in server block at line " + utils::to_str(line));
                        listen_seen = true;
                        if (args.size() != 1)
                            fail("'listen' expects one argument");
                        ...
                        if (args.size() != 1 || !utils::parse_size(args[0], srv.client_max_body_size))
                        ...
                        if (args.size() < 2)
                            fail("'error_page' expects codes and a path");
    ...
    for (size_t i = 0 ; i < servers.size() ; ++i){
        ...
        for (size_t j = 0 ; j < names.size() ; ++j) {
            ...
            if (!seen.insert(key).second)
                throw std::runtime_error("config error: duplicate listen+server_name '" + key + "'");
        }
    }
    return servers;
}
